...
for(isim in c(900:909)){
  overlap=60
  sim=paste0("Sim-overlap-",overlap,"-",isim)
...
# start the clock
ptm <- proc.time()
for(anc in c("CEU_10000","YRI_10000")){
...
TRN1="CEU_10000"
REF1=TRN1
for(TRN2 in c("YRI_10000","YRI_8000","YRI_6000","YRI_4000","YRI_2000")){
  for(isim in 900:909){
    REF2=TRN2
    ptm1 <- proc.time()
    sim=paste0("Sim-overlap-",overlap,"-",isim)
    ...
    re<-jlsSelectSnp(TRN1=TRN1,TRN2=TRN2, REF1=REF1, REF2=REF2, WORK.DIR = paste0(work.dir,"/",sim),REF.DIR = ref.dir,
                    ld.populations = ld.populations, N = N,shrink = c(0.75),lambda=sort(0.021-exp(seq(log(0.001), log(0.02), length.out=20)))[4:8], 
                    gamma=seq(0,1,.1),chrs=1:22)
    print(proc.time() - ptm1); flush.console()
  }
}
...
for(isim in 900:909){
  ...
  for(TRN2 in c("YRI_10000","YRI_8000","YRI_6000","YRI_4000","YRI_2000")){
    ...
    for(TUNE in c(TRN1,TRN2)){
      if(TUNE == "CEU_10000" & TRN2 != "YRI_10000")next
      re.jlsTuning<-jlsTuning(TRN1 = TRN1, TRN2 = TRN2, REF1= TRN1, REF2 = TRN2, TUNE = TUNE, WORK.DIR = paste0(work.dir,"/",sim), plink2 = plink2)
...
for(isim in 900:909){
  ...
  re.JLS.TESTING=NULL
  for(TST in c("CEU_10000","YRI_10000","YRI_8000","YRI_6000","YRI_4000","YRI_2000")){
    if(TST == TRN1){
      TRN2="YRI_10000"
    }else{
      TRN2=TST
    }
    re.jlsTesting<-jlsTesting(TRN1=TRN1,TRN2=TRN2,REF1=TRN1,REF2=TRN2,TST,combn=jls.TuningParameters[jls.TuningParameters$TUNE == TST,"combn"],
                            WORK.DIR=paste0(work.dir,"/",sim),plink2 = plink2)
