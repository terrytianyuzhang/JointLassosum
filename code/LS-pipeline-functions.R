...
lsSelectSNP <- function(TRN, REF, WORK.DIR,REF.DIR,ld.populations,N,s=seq(0.8,1.0,.05),lambda=exp(seq(log(0.001), log(0.02), length.out=20)), chrs=1:22){
  ...
  byLDBlock <- function(ld.block,TRN,REF,REF.DIR,N,s,lambda){
    ...
    ld.snp=ld.snp[ld.snp[,paste0(unlist(strsplit(REF,"_"))[1],".blk")] == ld.block,"ID"]
    ...
    system.time(re.chr <- lassosum.pipeline(cor=r, chr=trn.gwas$`#CHROM`, snp=trn.gwas$ID, pos=trn.gwas$POS, 
                                ...
                                ref.bfile=paste0(REF.DIR,"/",unlist(strsplit(REF,"_"))[1],".REF/CHR/",unlist(strsplit(REF,"_"))[1],".REF-chr",chr), 
                                test.bfile=NULL, #paste0(WORK.DIR,"/",TUNE,".TUNE/GNT/CHR/",TUNE,".TUNE-chr",chr),
                                s=s,lambda=lambda,
                                cluster = NULL,
                                sample=min(N[[REF]]$n.case+N[[REF]]$n.control,4000),
  ...
  ld.blocks=unique(ld.map[ld.map$CHROM %in% paste0("chr",chrs),paste0(unlist(strsplit(REF,"_"))[1],".blk")])
  
  re.ls.select<-mclapply(ld.blocks,byLDBlock,TRN=TRN,REF=REF,REF.DIR,N=N,s=s,lambda=lambda,mc.cores=min(length(ld.blocks),125),mc.preschedule = F, mc.silent=T)
...
lsTuning <- function(TRN, REF, TUNE, WORK.DIR, plink2){
  ...
  # get the scores for each chromosome
...
jlsSelectSnp <- function(TRN1,TRN2, REF1, REF2, WORK.DIR,REF.DIR,ld.populations,N,shrink=0.8,lambda=exp(seq(log(0.01), log(0.02), length.out=10)), gamma=seq(0,1,.1),chrs=1:22){
  ...
  jlsByGammaChr <- function(i.combn,combn,TRN1,TRN2,REF1,REF2,ld.populations,REF.DIR,N,shrink,lambda){
    ...
    fam=list()
    fam[[REF1]]=fread(paste0(REF.DIR,"/",unlist(strsplit(REF1,"_"))[1],".REF/CHR/",unlist(strsplit(REF1,"_"))[1],".REF-chr",chr,".fam"),header=F,data.table=F)
    fam[[REF1]]=fam[[REF1]][sample(min(nrow(fam[[REF1]]),4000),replace = F),1:2]
    
    fam[[REF2]]=fread(paste0(REF.DIR,"/",unlist(strsplit(REF2,"_"))[1],".REF/CHR/",unlist(strsplit(REF2,"_"))[1],".REF-chr",chr,".fam"),header=F,data.table=F)
    fam[[REF2]]=fam[[REF2]][sample(min(nrow(fam[[REF2]]),4000),replace = F),1:2]
    
    
    # run the joint lassosum
    re.jls<-mylassosum(cor1=r[[TRN1]], cor2=r[[TRN2]],
               fileName1 = paste0(REF.DIR,"/",unlist(strsplit(REF1,"_"))[1],".REF/CHR/",unlist(strsplit(REF1,"_"))[1],".REF-chr",chr), 
               fileName2 = paste0(REF.DIR,"/",unlist(strsplit(REF2,"_"))[1],".REF/CHR/",unlist(strsplit(REF2,"_"))[1],".REF-chr",chr),  
  ...
  system.time(RE.jls <- mclapply(1:nrow(combn),jlsByGammaChr,combn=combn,TRN1 = TRN1, TRN2 = TRN2, REF1 = REF1, REF2 = REF2,
                  ld.populations=ld.populations,REF.DIR=REF.DIR,N=N,shrink = shrink, lambda = lambda,mc.cores = 125, mc.preschedule = F))
...
#files=list.files("Data/REF.GNT/YRI_10000.REF/CHR",full.names = T)  
#for(file in files){
#  system(paste("mv",file,gsub("XREF-","REF-",file)))
#}  
